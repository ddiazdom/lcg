...
struct lm_parser {
    ...
    template<bool parse, class encoder_t, class map_type, class text_chunk_t>
        ...
        if constexpr (text_chunk_t::first_round){
            ...
            /*for(off_t i=ps;i<end_ps-6;i++){
                if(text_chunk.buffer[i]==65 && text_chunk.buffer[i+1]==65 &&
                   ...
                   text_chunk.buffer[i+4]==67 && text_chunk.buffer[i+5]==67 &&
                   text_chunk.buffer[i+6]==65 && text_chunk.buffer[i+7]==65){
                    std::cout<<"aparece en "<<i<<" -> "<<int(text_chunk.buffer[i+8])<<" "<<text_chunk.id<<std::endl;
                }
            }*/
        }
    ...
    // that is a local minima (i.e., a break). It also returns the number
    ...
    static size_t get_phrases(text_chunk_t& text_chunk, map_type& map){
        ...
        for(off_t str=-1;str<text_chunk.n_str;str++) {
            ps = text_chunk.str_buff_start(str);//leftmost byte of str
            end_ps = text_chunk.str_buff_end(str);
            //corner cases:
            //str=-1 and ps >= end_ps: the first symbol in the buffer is the start of str
            //str=n_str-1 and ps >= end_ps: the last symbol in the buffer is a sep_symbol
            if(ps<end_ps){
                n_mt_syms+=scan<false, typename text_chunk_t::decoder_t>(text_chunk, ps, end_ps, map);
            }
        }
            ...
            if(prev_hash!=r_order[i].hash){
                ...
                if((i-prev_pos)>1){
                    ...
                    std::sort(r_order.begin()+prev_pos, r_order.begin()+i, [&](auto a, auto b) -> bool{
                        ...
                        if(p_opts.p_round==0){
                            uint64_t sym_a = parsing_set[a.str_ptr+j];
                            uint64_t sym_b = parsing_set[b.str_ptr+j];
                            return hash_values_vector[sym_a]<hash_values_vector[sym_b];
                        }else{
...
#endif //SE_STRAT_LOCAL_MINIMA_PARSER_HPP
